...
## Acceptance Criteria
- [x] HomeView retains the 2×2 category grid as the primary entry point; the "Due today" hero number and standalone "New cards" panel are removed or refactored away from HomeView.
- [x] Tapping any category tile in the grid navigates to a new CategoryDetailView (or equivalent) scoped to that category.
- [x] CategoryDetailView shows two side-by-side tiles: "New: N" (count of cards not yet started for that category) and "Pending: N" (count of cards due/in-progress for that category).
- [x] Tapping "New: N" launches LearningQuizView for that category with the active-10 loop and 3-in-a-row graduation — this behaviour applies to all four categories (Countries, Rivers, Mountains, Seas).
- [x] Tapping "Pending: N" launches the existing review quiz flow for that category: MCQ for Rivers, Mountains, and Seas; for Countries it first shows the existing QuizModePickerView (Map quiz vs MCQ) before entering the chosen quiz.
- [x] The N counts in both tiles are accurate and update reactively when cards are learned or reviewed.
- [x] CategoryDetailView has a back-navigation affordance returning the user to HomeView.
- [x] No regressions in LearningQuizView, MultipleChoiceQuizView, or MapQuizView behaviour.
